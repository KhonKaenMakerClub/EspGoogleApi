...
bool HTTPBypass::begin(String url, String httpsFingerprint)
{
 ...
 * @param url String
 */
bool HTTPBypass::begin(String url)
{
...
bool HTTPBypass::begin(String host, uint16_t port, String uri)
{
    clear();
    _host = host;
    _port = port;
...
bool HTTPBypass::begin(String host, uint16_t port, String uri, bool https, String httpsFingerprint)
{
    if (https) {
...
bool HTTPBypass::begin(String host, uint16_t port, String uri, String httpsFingerprint)
{
    clear();
    _host = host;
    _port = port;
 ...
 * @param reuse bool
 */
void HTTPBypass::setReuse(bool reuse)
{
    _reuse = reuse;
}
 ...
 */
void HTTPBypass::setUserAgent(const String& userAgent)
{
    _userAgent = userAgent;
}

#ifdef USING_COOKIE
void HTTPBypass::setSession(bool usingSession) {
    _usingSession = usingSession;
}

void HTTPBypass::setCookie(String cookie){
    _cookie = cookie;
}
String HTTPBypass::getCookie(void){
    return _cookie;
}
#endif
/**
 ...
 */
void HTTPBypass::setAuthorization(const char * user, const char * password)
{
    if(user && password) {
        String auth = user;
        auth += ":";
 ...
 */
void HTTPBypass::setAuthorization(const char * auth)
{
    if(auth) {
 ...
 */
void HTTPBypass::setTimeout(uint16_t timeout)
{
    _tcpTimeout = timeout;
    if(connected()) {
 ...
 * @param timeout
 */
void HTTPBypass::useHTTP10(bool useHTTP10)
{
 ...
 * @return http code
 */
int HTTPBypass::POST(uint8_t * payload, size_t size)
{
    return sendRequest("POST", payload, size);
}

int HTTPBypass::POST(String payload)
{
 ...
 * @return
 */
int HTTPBypass::sendRequest(const char * type, String payload)
{
 ...
 */
int HTTPBypass::sendRequest(const char * type, uint8_t * payload, size_t size)
{
    // connect to server
    if(!connect()) {
 ...
 * @return -1 if no info or > 0 when Content-Length is set by server
 */
int HTTPBypass::sendRequest(const char * type, Stream * stream, size_t size)
{
 ...
 * @return WiFiClient
 */
WiFiClient& HTTPBypass::getStream(void)
{
    if(connected()) {
        return *_tcp;
    }
 ...
 * @return WiFiClient *
 */
WiFiClient* HTTPBypass::getStreamPtr(void)
{
    if(connected()) {
        return _tcp.get();
    }
 ...
 */
int HTTPBypass::writeToStream(Stream * stream)
{
 ...
 * @return String
 */
String HTTPBypass::getString(void)
{
 ...
 * @return String
 */
String HTTPBypass::errorToString(int error)
{
    switch(error) {
 ...
 * @param first
 */
void HTTPBypass::addHeader(const String& name, const String& value, bool first)
{
...
void HTTPBypass::collectHeaders(const char* headerKeys[], const size_t headerKeysCount)
{
...
String HTTPBypass::header(const char* name)
{
    for(size_t i = 0; i < _headerKeysCount; ++i) {
...
String HTTPBypass::header(size_t i)
{
...
String HTTPBypass::headerName(size_t i)
{
    if(i < _headerKeysCount) {
...
int HTTPBypass::headers()
{
    return _headerKeysCount;
}

bool HTTPBypass::hasHeader(const char* name)
{
    for(size_t i = 0; i < _headerKeysCount; ++i) {
...
bool HTTPBypass::connect(void)
{
    ...
    if (!_transportTraits) {
        DEBUG_HTTPCLIENT("[HTTP-Client] _transportTraits is null (HTTPBypass::begin not called?)\n");
        return false;
    }
 ...
 * @return status
 */
bool HTTPBypass::sendHeader(const char * type)
{
    if(!connected()) {
        return false;
    }
 ...
 */
int HTTPBypass::handleHeaderResponse()
{
    ...
    while(connected()) {
        ...
        if(len > 0) {
            ...
            //Serial.printf("[HTTP-Client][handleHeaderResponse] RX: '%s'\n", headerLine.c_str());
 ...
 */
int HTTPBypass::writeToStreamDataBlock(Stream * stream, int size)
{
    int buff_size = HTTP_TCP_BUFFER_SIZE;
    int len = size;
    int bytesWritten = 0;
 ...
 * @return error
 */
int HTTPBypass::returnError(int error)
{
    if(error < 0) {
